...
namespace ArcadeVP
{
    public class ArcadeVehicleController : MonoBehaviour
    {
        ...
        public void ProvideInputs(float _steer, float _accel, float _driftBrake, float _slowBrake)
        {
            ...
            driftInput = _driftBrake;  // Space/RightShift
            slowInput = _slowBrake;   // S/DownArrow
        }

        public void AudioManager()
        {
            engineSound.pitch = Mathf.Lerp(minPitch, MaxPitch, Mathf.Abs(carVelocity.z) / MaxSpeed);
            SkidSound.mute = !(Mathf.Abs(carVelocity.x) > 10 && grounded() && driftInput > 0.1f);
        }
        ...
        void FixedUpdate()
        {
            ...
            if (slowInput > 0.1f)
            {
                // e.g. lerp velocity towards zero on forward axis
                Vector3 localVel = carBody.transform.InverseTransformDirection(carBody.velocity);
                // only reduce forward/backwards
                float forward = localVel.z;
                forward = Mathf.MoveTowards(forward, 0f, slowInput * MaxSpeed * Time.deltaTime);
                Vector3 newVel = carBody.transform.TransformDirection(new Vector3(localVel.x, localVel.y, forward));
                rb.velocity = newVel;

                // skip the rest of drive logic this frame
                return;
            }
            if (grounded())
            {
                ...
                if (kartLike && driftInput > 0.1f) { TurnMultiplyer *= driftMultiplier; } //turn more if drifting
